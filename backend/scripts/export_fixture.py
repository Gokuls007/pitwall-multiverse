...
def main() -> None:
    ...
    # --- Decision-panel previews, one per candidate pit lap ---
    # Precomputed by actually calling `apply_decision`, not derived in the
    # frontend. The panel needs to respond instantly to a slider, and the
    # tempting shortcut is to recompute stint arithmetic client-side — which
    # is precisely the bug that produced a displayed tyre age the model never
    # used (see DECISIONS.md). Reading a table generated by the real code path
    # keeps the preview guaranteed consistent with what running the
    # counterfactual would actually do.
    #
    # The valid range is discovered by asking `apply_decision` rather than
    # reasoning about it: whatever it accepts is what the engine can run.
    candidates = []
    for candidate_lap in range(1, snapshot.total_laps + 1):
        candidate = ChangePitLap(
            driver=DECISION.driver, original_lap=DECISION.original_lap, new_lap=candidate_lap
        )
        try:
            candidate_overrides = apply_decision(snapshot, candidate)
        except ValueError:
            continue  # outside the range the engine can honestly represent
        if not candidate_overrides:
            continue
        candidate_excess = extrapolation_by_lap(snapshot, candidate_overrides)
        records = [
            (
                lap_number,
                candidate_overrides[(DECISION.driver, lap_number)].compound.value,
                candidate_overrides[(DECISION.driver, lap_number)].tyre_life,
                candidate_excess.get((DECISION.driver, lap_number), 0),
            )
            for lap_number in sorted(l for (d, l) in candidate_overrides if d == DECISION.driver)
        ]
        runs = stint_runs(records)
        # The stint the decision creates is the last run in the affected range.
        new_stint = runs[-1] if runs else None
        candidates.append(
            {
                "newLap": candidate_lap,
                "isReal": candidate_lap == DECISION.original_lap,
                ...
                "beyondEvidenceLaps": sum(1 for e in candidate_excess.values() if e > 0),
                "maxExcessLaps": max(candidate_excess.values(), default=0),
                "stints": runs,
            }
        )
